...
def fasta_to_matrix():
    ...
    seq_name_file = ['EBVdata/dsVIS_Data/dsVIS_neg_Test.mat',
                     'EBVdata/dsVIS_Data/dsVIS_pos_Test.mat',
                     'EBVdata/VISDB_Data/VISDB_neg_Test.mat',
                     'EBVdata/VISDB_Data/VISDB_pos_Test.mat']  #data file
    ...
    for name in seq_name_file:
        ...
        if 'VISDB_neg_Test' in name:
            print(name)
            y = []
            seq = []
            Liness_neg = loadmat(name)
            lines = Liness_neg['VISDB_neg_Test']
            for line in lines:

                seq.append(line)
            print('VISDB_neg_Test_processing!')
            global VISDB_neg_Test
            VISDB_neg_Test = seq_matrix(seq, dim)
            # global Data_neg_Test
            print('VISDB_neg_Test_end!')

        if 'VISDB_pos_Test' in name:
            print(name)
            y = []
            seq = []
            Liness_neg = loadmat(name)
            lines = Liness_neg['VISDB_pos_Test']
            for line in lines:
                seq.append(line)
            print('VISDB_pos_Test_processing!')
            global VISDB_pos_Test
            VISDB_pos_Test = seq_matrix(seq, dim)
            # global Data_pos_Test
            print('VISDB_pos_Test_end!')
    ...
    VISDB_Test_Data = np.concatenate([VISDB_pos_Test, VISDB_neg_Test])  # Test data
    VISDB_Test_Label = np.concatenate([np.ones(len(VISDB_pos_Test)), np.zeros(len(VISDB_neg_Test))])

    #np.save('HPVdata/Data_Train', Data_Train)
    #np.save('HPVdata/Label_Train', Label_Train)
    np.save('EBVdata/dsVIS_Data/dsVIS_Test_Data', dsVIS_Test_Data)
    np.save('EBVdata/dsVIS_Data/dsVIS_Test_Label', dsVIS_Test_Label)
    np.save('EBVdata/VISDB_Data/VISDB_Test_Data', VISDB_Test_Data)
    np.save('EBVdata/VISDB_Data/VISDB_Test_Label', VISDB_Test_Label)
